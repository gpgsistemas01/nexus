...
3. La matriz en memoria deriva permissions y scope.
...
El backend entrega `user.permissions` y `user.scope` en las vistas y mediante
...
derivadas. Para columnas, costos o flujo, el frontend consume `user.scope`, calculado
también por el backend; ya no interpreta directamente la relación rol/área.
