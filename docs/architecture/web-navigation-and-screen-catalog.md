...
| Administración | Catálogos (`/catalogos/:catalog`) | Administrar áreas, roles, presentaciones, unidades, motivos y estados desde una pantalla CRUD por catálogo y una implementación compartida por capas. | Elegir el catálogo en el submenú; listar, crear y editar; sólo para el administrador del sistema. | `src/views/pages/admin/catalogs/catalogsPage.ejs` |
