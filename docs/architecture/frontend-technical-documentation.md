...
| Catálogos | Poseen páginas parametrizadas bajo `/catalogos/:catalog` y además alimentan Select2 y formularios consumidores. | `pages/admin/catalogs/catalogPage.js` reutiliza una tabla y modal configurables; los adaptadores de `catalogs/{departments,roles,fulfillmentStatuses,presentations,reasons,unitMeasures}.js` conservan las lecturas de formularios. | La consulta mantiene el permiso específico de cada catálogo; `POST`, `PUT` y `DELETE` comparten `catalogs:manage`. | La configuración determina campos, endpoint y columnas sin duplicar el flujo CRUD. |
