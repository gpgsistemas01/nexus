...
| RN-033 | La trazabilidad de una merma debe conservar el documento que origina cada entrada o salida y su movimiento con saldos; mientras la existencia se administre como saldo fungible y no por lotes, una salida no debe atribuirse a una entrada específica. | Implementado | modelos `WasteStockEntry`, `WasteIssue`, `WasteMovement` y sus detalles en `prisma/schema.prisma` |
