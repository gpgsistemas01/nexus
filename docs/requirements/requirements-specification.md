...
| RF-CAT-006 | Almacén debe poder crear un material con presentación y unidad de medida; si captura SKU, éste debe ser único, y los límites y dimensiones deben ser válidos.<br><br>`CA-RF-CAT-006-1`: la identidad de material es nombre, presentación, unidad, base y altura; el proveedor pertenece a una relación separada.<br>`CA-RF-CAT-006-2`: si la identidad existe para otro proveedor, se reutiliza y se crea la nueva relación; si también existe esa relación, el alta se rechaza sin modificar stock. | Implementado | `src/routes/api/warehouse/materialApiRoute.js`, `src/services/warehouse/materials/materialService.js` |
| RF-CAT-007 | Almacén debe poder actualizar únicamente los datos generales admitidos de un material existente, incluido su estado activo, sin sustituir el flujo de ajuste de existencias.<br><br>`CA-RF-CAT-007-1`: activar o desactivar conserva la identidad, la relación proveedor-material, la existencia y la historia.<br>`CA-RF-CAT-007-2`: un material inactivo no se incorpora a compras, salidas o relaciones nuevas; los detalles de una salida registrados antes de desactivarlo pueden surtirse para completar el compromiso existente. | Implementado | `src/routes/api/warehouse/materialApiRoute.js`, `src/services/warehouse/materials/materialService.js` |
...
| RF-CAT-011 | Almacén debe poder actualizar los datos admitidos y el estado activo de un proveedor existente.<br><br>`CA-RF-CAT-011-1`: el cambio de estado forma parte de la edición; no elimina al proveedor ni sus relaciones o historia.<br>`CA-RF-CAT-011-2`: un proveedor inactivo no se usa en altas o detalles nuevos, sin impedir surtir detalles previamente comprometidos. | Implementado | `src/routes/api/warehouse/supplierApiRoute.js` |
...
| RF-CAT-015 | Almacén debe poder crear una merma a partir de un material y proveedor usados como plantilla; la merma resultante debe conservar snapshots propios.<br><br>`CA-RF-CAT-015-1`: la identidad de merma es proveedor, nombre, base y altura; si ya existe, el alta se rechaza sin modificar stock. | Implementado | `src/routes/api/warehouse/wasteApiRoute.js`, `src/services/warehouse/wastes/wasteService.js` |
...
| RF-CAT-017 | Almacén debe poder actualizar el nombre, el estado activo y los datos secundarios admitidos de una merma sin alterar directamente sus existencias.<br><br>`CA-RF-CAT-017-1`: activar o desactivar conserva la identidad, los snapshots, la existencia y la historia.<br>`CA-RF-CAT-017-2`: una merma inactiva no se agrega a salidas nuevas; si ya pertenece a una salida pendiente o parcial, puede surtirse para completar el compromiso con la validación normal de stock. | Implementado | `src/routes/api/warehouse/wasteApiRoute.js`, `src/views/pages/warehouse/wastes` |
...
| RF-REC-005 | Almacén debe poder editar los campos admitidos de una entrada no cancelada y agregar detalles nuevos, sin cambiar el proveedor ni reescribir detalles persistidos.<br><br>`CA-RF-REC-005-1`: los detalles nuevos requieren material y proveedor original activos; si están inactivos se rechazan sin impedir la edición admitida del encabezado ni una corrección histórica. | Implementado | `src/public/js/pages/warehouse/goodsReceipts/goodsReceiptForm.js`, `src/services/warehouse/goodsReceipts/goodsReceiptService.js` |
...
| RF-REC-007 | Almacén debe poder registrar una entrada con proveedor y detalles válidos; encabezado, detalles, incremento de existencias y movimientos deben confirmarse como una sola operación.<br><br>`CA-RF-REC-007-1`: proveedor y materiales deben estar activos al confirmar; un recurso inactivo rechaza la operación completa sin incrementar stock. | Implementado | `src/routes/api/warehouse/goodsReceiptApiRoute.js`, `src/dtos/goodsReceiptDTO.js`, `src/services/warehouse/goodsReceipts/goodsReceiptService.js` |
...
| RF-ISS-002 | Almacén debe poder surtir total o parcialmente un detalle con existencia suficiente; documento, acumulados, existencias, estados y movimiento deben actualizarse atómicamente.<br><br>`CA-RF-ISS-002-1`: si material o proveedor se desactivaron después de registrar el detalle, el pendiente puede surtirse desde el compromiso existente; la desactivación no crea otro uso ni cancela la salida. | Implementado | `src/services/inventory/movementService.js`, `src/services/warehouse/materials/supplierMaterialService.js` |
| RF-ISS-003 | Almacén debe poder devolver una cantidad surtida todavía retornable y enlazar el movimiento inverso con el documento y detalle originales.<br><br>`CA-RF-ISS-003-1`: una devolución parcial conserva el cumplimiento `Surtido` del detalle; devolver toda su cantidad surtida deriva `Cancelado` para ese detalle, sin ejecutar una operación separada de cancelación.<br><br>`CA-RF-ISS-003-2`: cuando, después de la devolución, todos los detalles tienen cumplimiento `Cancelado`, el sistema deriva cumplimiento `Cancelado` y estado documental `Cancelada` para el encabezado; un detalle no cancelado impide cancelar la salida completa. | Implementado | modelos `GoodsIssueReturn` y `MovementDetail`, `src/services/warehouse/goodsIssues/detailReturns/goodsIssueReturnService.js` |
| RF-ISS-004 | Almacén debe poder crear una salida de material con encabezado, contexto y detalles válidos, pendiente y sin descontar existencias.<br><br>`CA-RF-ISS-004-1`: cada material y proveedor debe estar activo; un recurso inactivo rechaza la salida sin crear detalles ni descontar stock. | Implementado | `src/routes/api/warehouse/goodsIssueApiRoute.js`, `src/views/pages/warehouse/goodsIssues` |
...
| RF-ISS-006 | Almacén debe poder agregar o actualizar detalles todavía modificables sin reescribir cantidades surtidas o devueltas.<br><br>`CA-RF-ISS-006-1`: todo recurso incorporado como detalle nuevo debe estar activo; esta condición no se vuelve a aplicar al surtimiento de un detalle persistido. | Implementado | `src/routes/api/warehouse/goodsIssueApiRoute.js`, `src/views/pages/warehouse/goodsIssues` |
...
| RF-WST-002 | Almacén debe poder crear una salida de merma con encabezado y detalles válidos dentro del estado inicial permitido.<br><br>`CA-RF-WST-002-1`: cada merma debe estar activa; una merma inactiva rechaza la salida sin crear detalles ni descontar stock. | Implementado | `src/routes/api/warehouse/wasteIssueApiRoute.js`, `src/public/js/pages/warehouse/wasteIssues/wasteIssueModal.js` |
| RF-WST-003 | Almacén debe poder surtir total o parcialmente un detalle de merma actualizando existencia, acumulados, estados y movimiento como una sola operación.<br><br>`CA-RF-WST-003-1`: una merma desactivada después de registrar el detalle puede completar su pendiente con la validación normal de stock; permanece bloqueada para salidas nuevas. | Implementado | `tests/integration/controllers/wasteIssueControllerDbTest.js` |
...
| RF-WST-005 | Almacén debe poder agregar o actualizar detalles de merma todavía modificables sin reescribir cantidades surtidas o devueltas.<br><br>`CA-RF-WST-005-1`: una merma incorporada como detalle nuevo debe estar activa; esta condición no se vuelve a aplicar al surtimiento del detalle persistido. | Implementado | `src/routes/api/warehouse/wasteIssueApiRoute.js`, `src/public/js/pages/warehouse/wasteIssues/wasteIssueModal.js` |
| RF-WST-006 | Almacén debe poder devolver una cantidad de merma todavía retornable actualizando existencia, acumulados, estados y movimiento inverso como una sola operación.<br><br>`CA-RF-WST-006-1`: una devolución parcial conserva el cumplimiento `Surtido` del detalle; devolver toda su cantidad surtida deriva `Cancelado` para ese detalle, sin ejecutar una operación separada de cancelación.<br><br>`CA-RF-WST-006-2`: cuando, después de la devolución, todos los detalles tienen cumplimiento `Cancelado`, el sistema deriva cumplimiento `Cancelado` y estado documental `Cancelada` para el encabezado; un detalle no cancelado impide cancelar la salida completa. | Implementado | `src/services/warehouse/wasteIssues/detailReturns/wasteIssueReturnService.js`, `tests/integration/controllers/wasteIssueControllerDbTest.js` |
...
| RF-REP-002 | Los módulos administrativo, comercial y de almacén pueden consultar los reportes registrados para su ámbito. Antes de exportar inventarios de materiales o mermas, el usuario puede incluir activos o con existencia, sólo activos o sólo registros con existencia; el alcance se combina con los filtros visibles. | Implementado | routers `reportApiRoute.js` de cada dominio; `reportExportDialog.js`; `warehouse/reportService.js` |
...
| RN-023 | Un material, merma o proveedor inactivo no debe incorporarse a una relación o detalle nuevo; su desactivación debe conservar identidad, stock e historia y no debe impedir completar un detalle registrado previamente, sujeto a las demás validaciones del surtimiento. | Implementado | `src/services/warehouse/goodsReceipts`, `src/services/warehouse/goodsIssues`, `src/services/warehouse/wasteIssues`, `src/services/warehouse/wastes` |
