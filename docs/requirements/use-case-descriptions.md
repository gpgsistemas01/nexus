...
| Flujos alternativos | **A1 — Datos inválidos (después del paso 3):**<br>1. **Nexus:** valida la información capturada, detecta campos incompletos, formatos incorrectos, relaciones no permitidas o cantidades fuera de las reglas del caso y los señala sin registrar cambios.<br>2. **Actor:** corrige la información indicada y vuelve a confirmar; continúa en el paso 4 del flujo principal.<br>**A2 — Identidad ya registrada (después del paso 4):**<br>1. **Nexus:** si también existe la relación con el mismo proveedor, rechaza el alta sin modificar la existencia; el actor localiza el inventario existente y usa el ajuste de stock. Si la identidad sólo existe para otro proveedor, Nexus la reutiliza y crea la nueva relación proveedor-material. |
| Excepciones | **E1 — Acceso rechazado (después del paso 1):**<br>1. **Nexus:** comprueba las precondiciones y la autorización, determina que alguna no se cumple y rechaza la solicitud sin modificar datos ni exponer información no autorizada; comunica el motivo.<br>2. **Actor:** reconoce el rechazo; termina el caso de uso. |
...
| Flujos alternativos | **A1 — Datos inválidos (después del paso 3):**<br>1. **Nexus:** valida la información capturada, detecta campos incompletos, formatos incorrectos, relaciones no permitidas o cantidades fuera de las reglas del caso y los señala sin registrar cambios.<br>2. **Actor:** corrige la información indicada y vuelve a confirmar; continúa en el paso 4 del flujo principal. |
| Excepciones | **E1 — Acceso rechazado (después del paso 1):**<br>1. **Nexus:** comprueba las precondiciones y la autorización, determina que alguna no se cumple y rechaza la solicitud sin modificar datos ni exponer información no autorizada; comunica el motivo.<br>2. **Actor:** reconoce el rechazo; termina el caso de uso. |
| Postcondiciones (éxito y fallo) | 1. **Éxito:** Actualización de datos generales admitidos.<br>2. **Éxito al desactivar:** el material conserva identidad, relaciones, stock e historia; queda excluido de operaciones nuevas, pero los detalles de salidas ya comprometidos pueden terminar de surtirse.<br>3. **Fallo:** Un rechazo no debe producir cambios parciales ni exponer información no autorizada. |
...
| Flujo principal | 1. **Actor:** abre «Agregar merma» y selecciona primero un proveedor **(ver E1)**.<br>2. **Nexus:** carga los materiales de ese proveedor que pueden utilizarse como plantilla.<br>3. **Actor:** elige el material, completa los datos propios de la merma y confirma **(ver A1)**.<br>4. **Nexus:** valida identidad, dimensiones, existencia y datos relacionados.<br>5. **Nexus:** crea la merma con sus propios datos históricos, registra su existencia inicial y confirma el alta. |
| Flujos alternativos | **A1 — Datos inválidos (después del paso 3):**<br>1. **Nexus:** valida la información capturada, detecta campos incompletos, formatos incorrectos, relaciones no permitidas o cantidades fuera de las reglas del caso y los señala sin registrar cambios.<br>2. **Actor:** corrige la información indicada y vuelve a confirmar; continúa en el paso 4 del flujo principal.<br>**A2 — Merma ya registrada (después del paso 4):**<br>1. **Nexus:** detecta la misma combinación de nombre, proveedor y dimensiones, rechaza el alta y no incrementa la existencia.<br>2. **Actor:** localiza la merma existente e inicia `CU-CAT-16` para establecer el nuevo stock total mediante un ajuste trazable. |
| Excepciones | **E1 — Acceso rechazado (después del paso 1):**<br>1. **Nexus:** comprueba las precondiciones y la autorización, determina que alguna no se cumple y rechaza la solicitud sin modificar datos ni exponer información no autorizada; comunica el motivo.<br>2. **Actor:** reconoce el rechazo; termina el caso de uso. |
...
| Flujo principal | 1. **Actor:** abre «Agregar compra» **(ver E1)**.<br>2. **Nexus:** muestra el formulario y carga proveedores, personas receptoras y materiales disponibles.<br>3. **Actor:** indica si el comprobante es factura o remisión; captura el número de factura sólo cuando corresponde, proveedor, persona receptora, fecha y hora de recepción y observaciones.<br>4. **Actor:** agrega cada material con su cantidad y costo por presentación, revisa los detalles y confirma **(ver A1)**.<br>5. **Nexus:** valida el tipo de comprobante, la factura cuando aplica, las relaciones, la fecha, las cantidades y los costos, y calcula los totales.<br>6. **Nexus:** registra compra y detalles, incrementa existencias y conserva los movimientos como una sola operación.<br>7. **Nexus:** actualiza la tabla y confirma el registro. |
| Flujos alternativos | **A1 — Datos inválidos (después del paso 3):**<br>1. **Nexus:** valida la información capturada, detecta campos incompletos, formatos incorrectos, relaciones no permitidas o cantidades fuera de las reglas del caso y los señala sin registrar cambios.<br>2. **Actor:** corrige la información indicada y vuelve a confirmar; continúa en el paso 4 del flujo principal.<br>**A2 — Material repetido en el detalle (después del paso 4):**<br>1. **Actor:** puede conservar renglones separados del mismo material cuando representan cantidades o costos por presentación distintos.<br>2. **Nexus:** procesa cada renglón, acumula todas sus cantidades en la existencia y conserva cada detalle por separado.<br>**A3 — Factura ya registrada (después del paso 5):**<br>1. **Nexus:** rechaza la combinación repetida de proveedor y factura e informa el folio existente.<br>2. **Actor:** abre esa compra para agregar los materiales faltantes; sólo crea otra compra si corresponde a otra factura o recepción.<br>**A4 — Material o proveedor inactivo (después del paso 5):**<br>1. **Nexus:** rechaza la compra sin crear detalles, movimientos ni cambios de stock.<br>2. **Actor:** selecciona recursos activos o solicita su reactivación cuando proceda. |
| Excepciones | **E1 — Acceso rechazado (después del paso 1):**<br>1. **Nexus:** comprueba las precondiciones y la autorización, determina que alguna no se cumple y rechaza la solicitud sin modificar datos ni exponer información no autorizada; comunica el motivo.<br>2. **Actor:** reconoce el rechazo; termina el caso de uso. |
...
| Flujos alternativos | **A1 — Datos inválidos (después del paso 3):**<br>1. **Nexus:** valida la información capturada, detecta campos incompletos, formatos incorrectos, relaciones no permitidas o cantidades fuera de las reglas del caso y los señala sin registrar cambios.<br>2. **Actor:** corrige la información indicada y vuelve a confirmar; continúa en el paso 4 del flujo principal.<br>**A2 — Recurso inactivo en un detalle nuevo (después del paso 4):**<br>1. **Nexus:** rechaza el material o proveedor inactivo sin agregar el detalle ni modificar stock.<br>2. **Actor:** selecciona recursos activos; el encabezado y los detalles persistidos permanecen sin cambios. |
| Excepciones | **E1 — Acceso rechazado (después del paso 1):**<br>1. **Nexus:** comprueba las precondiciones y la autorización, determina que alguna no se cumple y rechaza la solicitud sin modificar datos ni exponer información no autorizada; comunica el motivo.<br>2. **Actor:** reconoce el rechazo; termina el caso de uso. |
...
| Flujos alternativos | **A1 — Datos inválidos (después del paso 3):**<br>1. **Nexus:** valida la información capturada, detecta campos incompletos, formatos incorrectos, relaciones no permitidas o cantidades fuera de las reglas del caso y los señala sin registrar cambios.<br>2. **Actor:** corrige la información indicada y vuelve a confirmar; continúa en el paso 4 del flujo principal.<br>**A2 — Recurso desactivado después de registrar la salida (antes del paso 4):**<br>1. **Nexus:** conserva el detalle histórico y permite surtir su pendiente si existe stock suficiente; no habilita el recurso para operaciones nuevas.<br>2. **Actor:** completa el compromiso o lo deja pendiente hasta que exista una cancelación explícita. |
| Excepciones | **E1 — Acceso rechazado (después del paso 1):**<br>1. **Nexus:** comprueba las precondiciones y la autorización, determina que alguna no se cumple y rechaza la solicitud sin modificar datos ni exponer información no autorizada; comunica el motivo.<br>2. **Actor:** reconoce el rechazo; termina el caso de uso. |
...
| Flujo principal | 1. **Actor:** abre «Agregar salida de merma» **(ver E1)**.<br>2. **Nexus:** muestra el formulario y carga clientes, asesores, áreas, solicitantes y mermas disponibles.<br>3. **Actor:** selecciona cliente, asesor, área y solicitante; captura número de proyecto, fecha y hora de solicitud y observaciones.<br>4. **Actor:** agrega cada merma con su cantidad, revisa los detalles y selecciona «Guardar» **(ver A1)**. Si vuelve a agregar la misma merma, el formulario reemplaza su renglón con la cantidad capturada más reciente, sin sumarla.<br>5. **Nexus:** valida participantes, relaciones, mermas y cantidades y rechaza cualquier carga que todavía contenga una merma repetida.<br>6. **Nexus:** crea la salida pendiente sin descontar existencias, actualiza la tabla y confirma. |
| Flujos alternativos | **A1 — Datos inválidos (después del paso 3):**<br>1. **Nexus:** valida la información capturada, detecta campos incompletos, formatos incorrectos, relaciones no permitidas o cantidades fuera de las reglas del caso y los señala sin registrar cambios.<br>2. **Actor:** corrige la información indicada y vuelve a confirmar; continúa en el paso 4 del flujo principal.<br>**A2 — Merma inactiva (después del paso 5):**<br>1. **Nexus:** rechaza la salida sin crear detalles ni modificar stock.<br>2. **Actor:** selecciona una merma activa. |
| Excepciones | **E1 — Acceso rechazado (después del paso 1):**<br>1. **Nexus:** comprueba las precondiciones y la autorización, determina que alguna no se cumple y rechaza la solicitud sin modificar datos ni exponer información no autorizada; comunica el motivo.<br>2. **Actor:** reconoce el rechazo; termina el caso de uso. |
