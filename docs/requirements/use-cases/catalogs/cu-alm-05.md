...
| Flujo principal | 1. **Actor:** selecciona el material y abre «Ajustar existencia» **(ver E1)**.<br>2. **Nexus:** muestra la existencia actual, conserva el proveedor asociado requerido y muestra los campos de cantidad y motivo.<br>3. **Actor:** captura el ajuste y lo confirma **(ver A1)**.<br>4. **Nexus:** valida la autorización, el proveedor asociado, el motivo y la cantidad y registra el ajuste junto con la nueva existencia; actualiza las vistas de inventario y confirma el resultado. Además, guarda los cambios de la operación en la base de datos **(ver EBD)**. |
