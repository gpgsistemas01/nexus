...
| Disparador | Selecciona **Áreas** en el submenú **Catálogos auxiliares**. |
...
| Flujo principal | 1. **Actor:** selecciona **Áreas** en el submenú **Catálogos auxiliares** **(ver E1)**.<br>2. **Nexus:** comprueba que puede consultar Áreas y muestra su tabla; presenta **Nueva área** como acción principal para registrar un área **(ver A1)** **(ver A2)**. Además, consulta la información necesaria en la base de datos **(ver EBD)**.<br>3. **Actor:** selecciona **Nueva área**; termina `CU-CAT-09` y con esa selección dispara `CU-CAT-10` Crear área. |
| Flujos alternativos | **A1 — Permanecer en la consulta (después del paso 3):**<br>1. **Actor:** decide no iniciar el alta y revisa o busca entradas sin modificar datos.<br>2. **Nexus:** conserva la tabla de Áreas; termina el caso de uso.<br>**A2 — Editar una entrada (después del paso 2 del flujo principal):**<br>1. **Actor:** selecciona **Editar registro** en una entrada; termina `CU-CAT-09` y puede iniciar `CU-CAT-11` Editar área. El caso de edición comprueba nuevamente sus precondiciones y autorización. |
