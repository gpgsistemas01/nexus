...
import { personValidation } from '../../../src/validators/forms/personValidations.js';
...
describe('validaciones de campos obligatorios', () => {
  ...
  it('CRUD de personas: devuelve códigos constantes para accesos inválidos', async () => {
    const errors = await runValidation(personValidation, {
      fullName: 'Persona válida',
      accesses: [{ departmentId: 'invalid', roleId: 'invalid' }]
    });

    expect(errors).toEqual(expect.arrayContaining([
      expect.objectContaining({
        path: 'accesses[0].departmentId',
        msg: errorMap.personAccesses.INVALID_DEPARTMENT
      }),
      expect.objectContaining({
        path: 'accesses[0].roleId',
        msg: errorMap.personAccesses.INVALID_ROLE
      })
    ]));
  });
});
