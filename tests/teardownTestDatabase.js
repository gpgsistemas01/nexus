...
export default async function teardownTestDatabase() {
  ...
  const wasteIssueUnits = await prisma.unitMeasure.findMany({ where: { name: { startsWith: 'IT WasteIssue Unit ' } }, select: { id: true } });
  ...
  const adjustmentUnits = await prisma.unitMeasure.findMany({
    where: { name: { startsWith: 'IT Adjustment Unit ' } },
    select: { id: true }
  });
