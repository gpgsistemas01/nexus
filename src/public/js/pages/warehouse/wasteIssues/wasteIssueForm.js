...
const addWaste = () => {
    ...
    let { presentation, unitMeasure } = option.dataset;
    const { id, text, base, height } = option.dataset;
    presentation = JSON.parse(presentation || '{}');
    unitMeasure = JSON.parse(unitMeasure || '{}');
    ...
    const waste = {
        ...
        base,
        height,
        presentation: getPresentation({ presentation }),
        unitMeasure: getUnitMeasure({ unitMeasure }),
        quantity,
