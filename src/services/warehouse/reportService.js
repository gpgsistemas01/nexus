...
export const buildWasteReportSummary = (rows = []) => {
    ...
    rows.forEach((row) => {
        const parsedWidth = toNumber(row.base);
        const width = parsedWidth == null ? null : Number(parsedWidth);
        ...
        group.currentStock += Number(toNumber(row.currentStock) || 0);
        group.squareMeters += Number(toNumber(row.convertedQuantity) || 0);
