export const errorMap = {
    message: {
        LOGIN_ERROR: 'LOGIN_ERROR',
        VALIDATION_ERROR: 'VALIDATION_ERROR',
        INVALID_AUTH: 'INVALID_AUTH',
        FORBIDDEN: 'FORBIDDEN',
        INVALID_LINK: 'INVALID_LINK',
        SERVER_ERROR: 'SERVER_ERROR',
        DETECTED_REUSE: 'DETECTED_REUSE'
    },
    supplierId: {
        REQUIRED: 'SUPPLIER_ID_REQUIRED',
        INVALID_UUID: 'SUPPLIER_ID_INVALID_UUID'
    },
    materialId: {
        REQUIRED: 'MATERIAL_ID_REQUIRED',
        INVALID_UUID: 'MATERIAL_ID_INVALID_UUID'
    },
    supplierMaterialId: {
        REQUIRED: 'SUPPLIER_MATERIAL_ID_REQUIRED',
        INVALID_UUID: 'SUPPLIER_MATERIAL_ID_INVALID_UUID'
    },
    quantity: {
        REQUIRED: 'QUANTITY_REQUIRED',
        INVALID_NUMBER: 'QUANTITY_INVALID_NUMBER',
        TOO_LONG: 'QUANTITY_TOO_LONG'
    },
    returnQuantity: {
        REQUIRED: 'RETURN_QUANTITY_REQUIRED',
        INVALID_NUMBER: 'RETURN_QUANTITY_INVALID_NUMBER',
        TOO_LONG: 'RETURN_QUANTITY_TOO_LONG'
    },
    currentStock: {
        REQUIRED: 'CURRENT_STOCK_REQUIRED',
        INVALID_NUMBER: 'CURRENT_STOCK_INVALID_NUMBER',
        TOO_LONG: 'CURRENT_STOCK_TOO_LONG'
    },
    costPerUnitType: {
        REQUIRED: 'COST_PER_UNIT_REQUIRED',
        INVALID_NUMBER: 'COST_PER_UNIT_INVALID_NUMBER',
        TOO_LONG: 'COST_PER_UNIT_TOO_LONG'
    },
    receivedById: {
        REQUIRED: 'RECEIVED_BY_ID_REQUIRED',
        INVALID_UUID: 'RECEIVED_BY_ID_INVALID_UUID'
    },
    projectNumber: {
        REQUIRED: 'PROJECT_NUMBER_REQUIRED',
        INVALID_TYPE: 'PROJECT_NUMBER_INVALID_TYPE',
        TOO_LONG: 'PROJECT_NUMBER_TOO_LONG'
    },
    advisorId: {
        REQUIRED: 'ADVISOR_ID_REQUIRED',
        INVALID_UUID: 'ADVISOR_ID_INVALID_UUID'
    },
    clientId: {
        REQUIRED: 'CLIENT_ID_REQUIRED',
        INVALID_UUID: 'CLIENT_ID_INVALID_UUID'
    },
    departmentId: {
        REQUIRED: 'DEPARTMENT_ID_REQUIRED',
        INVALID_UUID: 'DEPARTMENT_ID_INVALID_UUID'
    },
    requesterId: {
        REQUIRED: 'REQUESTER_ID_REQUIRED',
        INVALID_UUID: 'REQUESTER_ID_INVALID_UUID'
    },
    presentationId: {
        REQUIRED: 'PRESENTATION_ID_REQUIRED',
        INVALID_UUID: 'PRESENTATION_ID_INVALID_UUID'
    },
    unitMeasureId: {
        REQUIRED: 'UNIT_MEASURE_ID_REQUIRED',
        INVALID_UUID: 'UNIT_MEASURE_ID_INVALID_UUID'
    },
    reasonId: {
        REQUIRED: 'REASON_ID_REQUIRED',
        INVALID_UUID: 'REASON_ID_INVALID_UUID'
    },
    roleId: {
        REQUIRED: 'ROLE_ID_REQUIRED',
        INVALID_UUID: 'ROLE_ID_INVALID_UUID'
    },
    username: {
        REQUIRED: 'USERNAME_REQUIRED',
        INVALID_TYPE: 'USERNAME_INVALID_TYPE',
        NO_SPACES: 'USERNAME_NO_SPACES',
        INVALID_FORMAT: 'USERNAME_INVALID_FORMAT',
        TOO_LONG: 'USERNAME_TOO_LONG'
    },
    password: {
        REQUIRED: 'PASSWORD_REQUIRED',
        INVALID_TYPE: 'PASSWORD_INVALID_TYPE',
        NEEDS_NUMBER: 'PASSWORD_NEEDS_NUMBER',
        NEEDS_UPPERCASE: 'PASSWORD_NEEDS_UPPERCASE',
        INVALID_FORMAT: 'PASSWORD_INVALID_FORMAT',
        TOO_SHORT: 'PASSWORD_TOO_SHORT',
        TOO_LONG: 'PASSWORD_TOO_LONG',
    },
    name: {
        REQUIRED: 'NAME_REQUIRED',
        INVALID_TYPE: 'NAME_INVALID_TYPE',
        TOO_LONG: (value) => ({
            code: 'NAME_TOO_LONG',
            meta: { value }
        }),
        INVALID_FORMAT: 'NAME_INVALID_FORMAT'
    },
    invoice: {
        REQUIRED: 'INVOICE_REQUIRED',
        INVALID_TYPE: 'INVOICE_INVALID_TYPE',
        TOO_LONG: (value) => ({
            code: 'INVOICE_TOO_LONG',
            meta: { value }
        }),
        INVALID_FORMAT: 'INVOICE_INVALID_FORMAT'
    },
    newStock: {
        REQUIRED: 'NEW_STOCK_REQUIRED',
        INVALID_NUMBER: 'NEW_STOCK_INVALID_NUMBER',
        TOO_LONG: 'NEW_STOCK_TOO_LONG'
    },
    minStock: {
        REQUIRED: 'MIN_STOCK_REQUIRED',
        INVALID_NUMBER: 'MIN_STOCK_INVALID_NUMBER',
        MIN_GREATER_THAN_MAX: 'MIN_STOCK_GREATER_THAN_MAX'
    },
    maxUnitCost: {
        REQUIRED: 'MAX_UNIT_COST_REQUIRED',
        INVALID_NUMBER: 'MAX_UNIT_COST_INVALID_NUMBER',
        TOO_LONG: 'MAX_UNIT_COST_TOO_LONG'
    },
    receptionDate: {
        REQUIRED: 'RECEPTION_DATE_REQUIRED',
        INVALID_FORMAT: 'RECEPTION_DATE_INVALID_FORMAT'
    },
    requestDate: {
        REQUIRED: 'REQUEST_DATE_REQUIRED',
        INVALID_FORMAT: 'REQUEST_DATE_INVALID_FORMAT'
    },
    base: {
        REQUIRED: 'BASE_REQUIRED',
        INVALID_NUMBER: 'BASE_INVALID_NUMBER',
        TOO_LONG: 'BASE_TOO_LONG'
    },
    height: {
        REQUIRED: 'HEIGHT_REQUIRED',
        INVALID_NUMBER: 'HEIGHT_INVALID_NUMBER',
        TOO_LONG: 'HEIGHT_TOO_LONG'
    },
    observations: {
        INVALID_TYPE: 'OBSERVATIONS_INVALID_TYPE',
        TOO_LONG: (value) => ({
            code: 'OBSERVATIONS_TOO_LONG',
            meta: { value }
        }),
        INVALID_FORMAT: 'OBSERVATIONS_INVALID_FORMAT'
    },
    isInvoiced: {
        REQUIRED: 'INVOICED_REQUIRED',
        INVALID_BOOLEAN: 'INVOICED_INVALID_BOOLEAN',
    },
    isActive: {
        REQUIRED: 'ACTIVE_REQUIRED',
        INVALID_BOOLEAN: 'ACTIVE_INVALID_BOOLEAN'
    },
    isSupplied: {
        REQUIRED: 'SUPPLIED_REQUIRED',
        INVALID_BOOLEAN: 'SUPPLIED_INVALID_BOOLEAN'
    },
    details: {
        REQUIRED: 'DETAILS_REQUIRED',
        INVALID_FORMAT_SUPPLIER: 'DETAILS_INVALID_FORMAT_SUPPLIER',
        INVALID_FORMAT_REQUIRED: 'DETAILS_INVALID_FORMAT_REQUIRED',
        REQUIRED_QUANTITY: 'REQUIRED_QUANTITY',
        INVALID_FORMAT_QUANTITY: 'DETAILS_INVALID_FORMAT_QUANTITY',
        INVALID_FORMAT_COST_PER_UNIT_TYPE: 'DETAILS_INVALID_FORMAT_COST_PER_UNIT_TYPE',
        INVALID_FORMAT_ID: 'DETAILS_INVALID_FORMAT_ID'
    },
    departments: {
        INVALID_FORMAT: 'DEPARTMENTS_INVALID_FORMAT',
        REQUIRED: 'DEPARTMENTS_REQUIRED',
    },
    catalog: {
        NOT_FOUND: 'CATALOG_NOT_FOUND',
        ENTRY_NOT_FOUND: 'CATALOG_ENTRY_NOT_FOUND',
        VALIDATION_ERROR: 'CATALOG_VALIDATION_ERROR'
    },
    personAccesses: {
        REQUIRED: 'PERSON_ACCESSES_REQUIRED',
        DUPLICATE_DEPARTMENT: 'PERSON_ACCESSES_DUPLICATE_DEPARTMENT',
        INVALID_DEPARTMENT: 'PERSON_ACCESSES_INVALID_DEPARTMENT',
        INVALID_ROLE: 'PERSON_ACCESSES_INVALID_ROLE'
    }
};

export const successCodeMessages = {
    CREATED_ACCOUNT: 'CREATED_ACCOUNT',
    UPDATED_ACCOUNT: 'UPDATED_ACCOUNT',
    CREATED_PERSON: 'CREATED_PERSON',
    UPDATED_PERSON: 'UPDATED_PERSON',
    SUCCESS_LOGIN: 'SUCCESS_LOGIN',
    SUCCESS_LOGOUT: 'SUCCESS_LOGOUT',
    CREATED_MATERIAL: 'CREATED_MATERIAL',
    UPDATED_MATERIAL: 'UPDATED_MATERIAL',
    DELETED_MATERIAL: 'DELETED_MATERIAL',
    CREATED_SUPPLIER: 'CREATED_SUPPLIER',
    UPDATED_SUPPLIER: 'UPDATED_SUPPLIER',
    CREATED_GOODS_RECEIPT: 'CREATED_GOODS_RECEIPT',
    UPDATED_GOODS_RECEIPT: 'UPDATED_GOODS_RECEIPT',
    CREATED_GOODS_ISSUE: 'CREATED_GOODS_ISSUE',
    UPDATED_GOODS_ISSUE: 'UPDATED_GOODS_ISSUE',
    CREATED_CLIENT: 'CREATED_CLIENT',
    CREATED_CATALOG_ENTRY: 'CREATED_CATALOG_ENTRY',
    CREATED_USER: 'CREATED_USER',
    UPDATED_USER: 'UPDATED_USER',
    UPDATED_USER_PASSWORD: 'UPDATED_USER_PASSWORD',
    UPDATED_CLIENT: 'UPDATED_CLIENT',
    UPDATED_CATALOG_ENTRY: 'UPDATED_CATALOG_ENTRY',
    CREATED_WASTE: 'CREATED_WASTE',
    UPDATED_WASTE: 'UPDATED_WASTE',
    DELETED_WASTE: 'DELETED_WASTE',
    CREATED_WASTE_ISSUE: 'CREATED_WASTE_ISSUE',
    UPDATED_WASTE_ISSUE: 'UPDATED_WASTE_ISSUE',
}
